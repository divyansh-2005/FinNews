            ...
            <div className={styles.textContent}>
              <div className={styles.companyTime}>
                <span className={styles.companyName}>
                  {news.title.includes("OpenAI")
                    ? "OpenAI"
                    : news.title.includes("Swiggy")
                    ? "Swiggy"
                    : news.title.includes("SpiceJet")
                    ? "SpiceJet"
                    : news.title.includes("Apple")
                    ? "Apple"
                    : news.title.includes("Zerodha")
                    ? "Zerodha"
                    : news.title.includes("Zepto")
                    ? "Zepto"
                    : news.title.includes("Gemini")
                    ? "Gemini"
                    : news.title.includes("NASA")
                    ? "NASA"
                    : news.title.includes("Nasa")
                    ? "Nasa"
                    : news.title.includes("Samsung")
                    ? "Samsung"
                    : "Unknown"}
                </span>
                <span className={styles.time}>{news.publishedAt} </span>
              </div>
              <h1 className={styles.headingNews}>{news.title}</h1>
              <div className={styles.description}>{news.description}</div>
            </div>
            <div className={styles.image}>
              <img src={news.image} alt="containt-img" />
            </div>
          </div>
        ))}
      </div>
